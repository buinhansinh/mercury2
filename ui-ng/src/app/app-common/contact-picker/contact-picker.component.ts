...
export class ContactPickerComponent implements OnInit {
    ...
    @Output() contactPicked = new EventEmitter<Contact>();
    ...
    ngOnInit() {
        this.contacts$ = this.form.valueChanges
            .pipe(
    ...
    onSearch(terms: string | Contact): Observable<Contact[]> {
        return (typeof terms === 'string') ? this.contactService.search(<string>terms) : of([]);
    }
